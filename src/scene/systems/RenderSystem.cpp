...
void DxEngine::SceneNS::RenderSystem::Draw(entt::registry& registry)
{
	...
	for (auto entity : view) {
		if (view.get<Components::Renderable>(entity).render = true) {
			...
			/*if (!renderer.guiContext->WantCaptureMouse) {
				renderer.camera->EditorCameraUpdate();
			}*/
			...
			//renderer.deviceContext->VSSetConstantBuffers(1, 1, renderer.renderStates.get()->constantBuffer.GetAddressOf());
			...
			//renderer.deviceContext->PSSetConstantBuffers(1, 1, renderer.renderStates.get()->constantBuffer.GetAddressOf());
			...
			//renderer.m .model(renderer.deviceContext.Get());
