...
ID3D11Buffer* g_pConstantBufferBones = NULL;
...
void InitConstantBuffer(std::vector<Mesh>& allObjects) {
	...
	HR = g_pd3dDevice->CreateBuffer(&bd1, nullptr, &g_pConstantBufferBones);
	g_pd3dDeviceContext->VSSetConstantBuffers(2, 1, &g_pConstantBufferBones);
